...
class TestIntegration:
    ...
    def test_complete_booking_flow(self, test_client):
        ...
        user1_response = test_client.post(
            "/users/", json={"nombre": "Juan Pérez", "email": "juan@example.com"}
        )
        ...
        user2_response = test_client.post(
            "/users/", json={"nombre": "María García", "email": "maria@example.com"}
        )
        ...
        reservation1_response = test_client.post(
            "/reservations/",
            json={
                "userId": user1["id"],
                "roomId": room["id"],
        ...
        reservation2_response = test_client.post(
            "/reservations/",
            json={
                "userId": user2["id"],
                "roomId": room["id"],
        ...
        reservation3_response = test_client.post(
            "/reservations/",
            json={
                "userId": user2["id"],
                "roomId": room["id"],
        ...
        availability_response = test_client.get(
            f"/rooms/{room['id']}/availability", params={"date": "2025-12-15"}
        )
        ...
        reservation4_response = test_client.post(
            "/reservations/",
            json={
                "userId": user1["id"],
                "roomId": room["id"],
    ...
    def test_user_email_uniqueness(self, test_client):
        ...
        test_client.post(
            "/users/", json={"nombre": "Carlos López", "email": "carlos@example.com"}
        )

        # Intentar crear segundo usuario con mismo email
        duplicate_response = test_client.post(
            "/users/",
            json={"nombre": "Carlos Duplicado", "email": "carlos@example.com"},
        )
    ...
    def test_room_capacity_validation(self, test_client):
        ...
        """
        invalid_room_response = test_client.post(
            "/rooms/",
            json={
